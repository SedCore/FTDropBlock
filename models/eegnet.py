...
def EEGNetv4(nb_classes, Chans=64, Samples=128, dropoutRate=0.5, kernLength=64, F1=8, D=2, F2=16, norm_rate=0.25, dropType='Dropout',block=15):
    ...
    block1 = FTDropBlock2D(dropoutRate, block_size=block, tensorformat='NCHW')(block1) if dropType == 'FTDropBlock2D' else Dropout(dropoutRate)(block1)

    block2 = SeparableConv2D(F2, (1, 16), use_bias=False, padding='same')(block1)
    block2 = BatchNormalization(axis=1)(block2)
    block2 = Activation('elu')(block2)
    block2 = AveragePooling2D((1, 8))(block2)
    block2 = FTDropBlock2D(dropoutRate, block_size=block, tensorformat='NCHW')(block2) if dropType == 'FTDropBlock2D' else Dropout(dropoutRate)(block2)
